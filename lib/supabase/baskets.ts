...
export const createBasket = async (name: string) => {
  // Create basket
  const { data: newBasket, error: newBasketError }: any = await supabase
    .from("baskets")
    .insert({ name })
    .select("*");
  ...
  const { error } = await supabase
    .from("users_baskets")
    .insert({ userId: userId, basketId: newBasket[0].id })
    .select("*");
...
export const getBaskets = async () => {
  ...
  const { data: userBaskets, error: userBasketsError } = await supabase
    .from("users_baskets")
    .select("*")
    .eq("userId", userId);
...
export const getBasketById = async (basketId: number) => {
  const { data, error } = await supabase
    .from("baskets")
    .select("*, basket_items(*)") // Include basket items
    .eq("id", basketId)
    .single();

  if (!data || error) {
    showToast("error", error?.message || "Basket not found");
    return { data: null, error };
  }
